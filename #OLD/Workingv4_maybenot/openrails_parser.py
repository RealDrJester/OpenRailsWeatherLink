...
class OpenRailsParser:
    ...
    def get_activity_details(self, act_path_str):
        details = {"description": "N/A", "briefing": "N/A", "path_id": None, "existing_weather": ""}
        ...
        if weather_events: details["existing_weather"] = "\n".join(weather_events)
        else: details["existing_weather"] = "No ORTSWeatherChange events found in this file."
        return details
    ...
    def get_activity_path_coords(self, route_path_str, path_id):
        ...
        self.log(f"[INFO] Parsing activity path: {pat_path.name}"); content, _ = self._read_file(pat_path)
        ...
        if not pdp_list or not path_indices: self.log(f"[WARN] No valid path data found in {pat_path.name}"); return []
        ...
        self.log(f"[INFO] Generated map path with {len(path_coords)} points."); return path_coords
    def modify_and_save_activity(self, route_path, original_path_str, dat_content, act_events_content, date_obj=None):
        original_path = Path(original_path_str)
        try:
            date_str = date_obj.strftime("%Y%m%d") if date_obj else datetime.now().strftime("%Y%m%d")
            filename_suffix = f"{APP_SUFFIX}.{date_str}"
            ...
            dat_path = Path(route_path) / "weathertransitions.dat"
            existing_dat_content = ""
            if dat_path.exists():
                existing_dat_content, _ = self._read_file(dat_path)
                content_to_check = re.sub(r'ORTSWeatherChange\s*\(\s*Name\s*\(\s*"WTHLINK_Hour_\d+"\s*\).*?\)', '', existing_dat_content, flags=re.DOTALL)
                clean_check = re.sub(r'ORTSWeatherChangeList\s*\(|\)|\s', '', content_to_check, flags=re.IGNORECASE)
                if len(clean_check) > 0:
                    bak_path = dat_path.with_suffix('.dat.bak')
                    shutil.copy2(dat_path, bak_path)
                    self.log(f"  > Backed up existing {dat_path.name} to {bak_path.name}")
            
            if existing_dat_content:
                existing_dat_content = re.sub(r'^\s*ORTSWeatherChangeList\s*\(', '', existing_dat_content, flags=re.IGNORECASE | re.DOTALL).strip()
                existing_dat_content = re.sub(r'\)\s*$', '', existing_dat_content).strip()
                existing_dat_content = re.sub(r'\s*ORTSWeatherChange\s*\(\s*Name\s*\(\s*"WTHLINK_Hour_\d+"\s*\).*?\)', '', existing_dat_content, flags=re.DOTALL)
            
            new_dat_content = dat_content.replace('ORTSWeatherChangeList (', '').replace(')', '').strip()
            full_dat_content = f"ORTSWeatherChangeList (\n{existing_dat_content}\n{new_dat_content}\n)"
            with open(dat_path, 'w', encoding='utf-8') as f: f.write(full_dat_content)
            self.log(f"  > Updated weather presets in: {dat_path}")
            ...
            if name_match:
                ...
                new_activity_name = f"{original_name} [{APP_SUFFIX}.{date_str}]"
            ...
            clean_events_content = re.sub(r'\s*EventCategoryTime\s*\(\s*Name\s*\(\s*WTHLINK_Hour_\d+.*?\)\s*.*?\)', '', events_match.group(2), flags=re.DOTALL | re.IGNORECASE).strip()
