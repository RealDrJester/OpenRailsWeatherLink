...
class MainApplication(tk.Tk):
    ...
    def update_activity_info(self):
        ...
        lat, lon = self.parser.find_route_start_location(route_info)
        self.found_coords = (lat, lon)
        if lat is not None: self.update_weather_info(lat, lon)
    ...
    def generate_weather_worker(self):
        ...
        self.log(f"  > {msg}")
        route_info = self.current_route_data.get(self.route_listbox.get(self.route_listbox.curselection()[0]))
        new_path, msg = self.parser.modify_and_save_activity(route_info['path'], self.selected_activity_path, weather_events, date_obj)
