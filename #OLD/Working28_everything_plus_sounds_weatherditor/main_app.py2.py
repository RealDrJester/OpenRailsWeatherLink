...
class MainApplication(tk.Tk):
    ...
    def setup_ui(self):
        ...
        editor_frame = ttk.Frame(action_frame)
        editor_frame.pack(fill=tk.X, pady=5)
        ttk.Button(editor_frame, text="Manual Weather Editor...", command=self.show_manual_editor).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))
        self.edit_weather_button = ttk.Button(editor_frame, text="Load & Edit Weather File...", state=tk.DISABLED, command=self.load_and_edit_weather_file)
        self.edit_weather_button.pack(side=tk.LEFT, expand=True, fill=tk.X)
    ...
    def _filter_routes(self, event=None):
        ...
        for name in self._all_routes_sorted:
            if filter_text in name.lower(): self.route_listbox.insert(tk.END, name)
    ...
    def update_weather_info(self, lat, lon, route_info, date_obj=None):
        ...
        weather_points = [(lat, lon)]
        # --- FIX: Correctly select only start, middle, and end points for long routes ---
        if self.path_dist > 100000 and self.path_coords_cache and len(self.path_coords_cache) > 2:
            start_point = self.path_coords_cache[0][0]
            mid_dist = self.path_dist / 2
            mid_point = min(self.path_coords_cache, key=lambda p: abs(p[1] - mid_dist))[0]
            end_point = self.path_coords_cache[-1][0]
            weather_points = [start_point, mid_point, end_point]
            self.log(f"[Info] Long route detected. Using 3 weather points.")

        data = self.weather.get_weather_data(weather_points, date_obj)
        if not self.shutdown_event.is_set(): self.after(0, lambda: self._update_weather_widgets(data, date_obj))
        if not self.shutdown_event.is_set(): self.after(0, self.stop_loading)
    ...
    def _update_weather_widgets(self, data, date_obj):
        ...
        for i in range(min(24, len(hourly.get('time', [])))):
            ...
            cond = self.weather.WMO_CODES.get(int(hourly['weathercode'][i]), 'N/A') if 'weathercode' in hourly and hourly['weathercode'][i] is not None else "N/A"
    ...
    def run_generation_thread(self, chaotic=False): 
        threading.Thread(target=self.generate_weather_worker, args=(chaotic,), daemon=True).start()
    ...
    def generate_weather_worker(self, chaotic=False):
        ...
        if chaotic:
            ...
            date_obj = self.historical_selection['date'] if self.historical_selection else datetime.now().date()
            ...
            force_snow = self.force_snow_var.get(); add_thunder = self.add_thunder_var.get(); add_wind = self.add_wind_var.get(); add_rain = self.add_rain_var.get()
            ...
            weather_events, msg = self.weather.create_weather_events_string(path_coords_for_api, self.path_dist, season, force_snow, date_obj, start_hour, add_thunder, add_wind, add_rain, self.sound_manager, route_info['path'])
            
        if self.shutdown_event.is_set(): return
        if not weather_events: self.log(f"[ERROR] Weather event string creation failed: {msg}"); self.after(0, self.stop_loading); return
        self.log(f"[Info] {msg}")
        
        new_path, msg = self.parser.modify_and_save_activity(self.selected_activity_path, weather_events, date_obj, chaotic, season=season)
        if new_path:
            self.log(f"[Success] {msg}\n  > Saved to: {Path(new_path).name}"); self.after(0, lambda: messagebox.showinfo("Success", f"New activity file created:\n\n{Path(new_path).name}"))
        else:
    ...
    def run_global_cleanup(self):
        ...
        if messagebox.askyesno("Confirm GLOBAL Cleanup", msg, icon='warning'):
            ...
            self.stop_loading()
            messagebox.showinfo("Global Cleanup Complete