...
class MainApplication(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.force_snow_var = tk.BooleanVar(value=True); self.force_snow_check = ttk.Checkbutton(action_frame, text="Force Snow in Winter", variable=self.force_snow_var); self.force_snow_check.pack(fill=tk.X, pady=5)
    ...
    def show_debug_info(self):
        try:
            info = (
                ...
                f"Path Coords Cached: {len(self.path_coords_cache) if self.path_coords_cache else 0}\n"
    ...
    def update_weather_info(self, lat, lon, route_info, date_obj=None):
        ...
        if self.path_dist and self.path_dist > 50000 and self.path_coords_cache and len(self.path_coords_cache) > 2:
            ...
            self.weather_fetch_points = [start_point, mid_point, end_point]
            self.log(f"[Info] Long route detected (>{self.path_dist/1000:.1f}km). Using 3 weather points.")
    ...
    def _update_weather_widgets(self, data, date_obj):
        ...
        if self.historical_selection:
            ...
            self.generate_button.config(text="Generate Historical Activity")
        else:
            self.current_weather_button.config(state=tk.DISABLED)
            self.generate_button.config(text="Generate Live Weather Activity")
