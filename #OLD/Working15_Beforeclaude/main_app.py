...
from ui_components import AboutWindow, ForecastChart, DateSelectionWindow, TKCALENDAR_AVAILABLE, SettingsWindow, ManualWeatherEditor
...
class MainApplication(tk.Tk):
    ...
    def refresh_sound_files(self):
        self.log("[Info] User triggered sound file rescan.")
        self.sound_manager.discover_sounds()
        messagebox.showinfo("Rescan Complete", "The `user_sounds` folder has been rescanned for audio files.")
    ...
    def run_generation_thread(self, chaotic=False, preset=None): 
        self.log("[Debug] Generation thread started.")
        threading.Thread(target=self.generate_weather_worker, args=(chaotic, self.preset_menu_var.get()), daemon=True).start()
    ...
    def generate_weather_worker(self, chaotic=False, preset_name=None):
        ...
        if lat is None: self.log("[ERROR] Generation failed: Coordinates not found."); self.after(0, self.stop_loading); self.after(0, lambda: self.generate_button.config(state=tk.NORMAL)); return
        ...
        self.log("[Info] Provisioning all necessary sound files for the route...")
        self.sound_manager.provision_sound_files_for_route(route_path)
