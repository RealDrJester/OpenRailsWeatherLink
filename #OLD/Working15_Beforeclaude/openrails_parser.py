...
class OpenRailsParser:
    def __init__(self, content_path_str, log_callback=print):
        ...
        self.log(f"[Debug] Parser initialized for content path: {self.content_path}")
    def _read_file(self, path):
        self.log(f"[Debug] Reading file: {path}")
        try:
            with open(path, 'r', encoding='utf-16-le', errors='strict') as f:
                self.log("[Debug] File encoding detected as utf-16-le.")
                return f.read(), 'utf-16-le'
        except (UnicodeError, UnicodeDecodeError):
            try:
                with open(path, 'r', encoding='utf-8-sig', errors='strict') as f:
                    self.log("[Debug] File encoding detected as utf-8-sig.")
                    return f.read(), 'utf-8-sig'
            except Exception as e:
                self.log(f"[ERROR] Could not read file {path} with any known encoding. Reason: {e}")
                return None, None
        except Exception as e:
            self.log(f"[ERROR] Could not open file {path}. Reason: {e}")
            return None, None
    def load_track_nodes_for_route(self, trk_path_str):
        self.current_track_nodes = {}
        pass
    def get_all_routes(self):
        self.log("[Debug] Searching for routes...")
        routes = {}
        if not self.routes_path.is_dir():
            self.log(f"[ERROR] ROUTES directory not found at {self.routes_path}")
            return {}
        for trk_path in sorted(self.routes_path.glob("**/*.trk")):
            ...
            if name_match and route_id_match:
                routes[name_match.group(1)] = {"id": route_id_match.group(1).strip(), "path": str(trk_path.parent), "trk_path": str(trk_path)}
        self.log(f"[Info] Found {len(routes)} routes.")
        return routes
        
    def add_sounds_to_trk_file(self, trk_path_str, sound_manager):
        self.log(f"[Info] Updating TRK override file for sound sources: {trk_path_str}")
        original_trk_path = Path(trk_path_str)
        route_path = original_trk_path.parent
        or_folder = route_path / "OpenRails"
        or_folder.mkdir(exist_ok=True)
        
        override_trk_path = or_folder / original_trk_path.name
        
        all_sms_files = []
        for category, sounds in sound_manager.sounds.items():
            if sounds and sounds[0]['looping']:
                all_sms_files.append(f"WEATHERLINK_{category}.sms")

        if not all_sms_files:
            self.log("[Info] No looping sound files to add to TRK override.")
            return True
            
        include_line = f'include ( "..\\\\{original_trk_path.name}" )'
        content = ""
        encoding = 'utf-8-sig'

        if override_trk_path.exists():
            self.log(f"[Debug] Existing TRK override file found: '{override_trk_path.name}'")
            backup_path = override_trk_path.with_suffix('.trk.bak')
            if not backup_path.exists():
                shutil.copy2(override_trk_path, backup_path)
                self.log(f"[Success] Created backup: '{backup_path.name}'")
            
            try:
                with open(override_trk_path, 'r', encoding=encoding, errors='strict') as f:
                    content = f.read()
            except:
                 with open(override_trk_path, 'r', encoding='utf-16-le', errors='strict') as f:
                    content = f.read()

            if include_line not in content:
                content = f"{include_line}\n{content}"
        else:
            self.log("[Debug] No TRK override file found. Creating new one.")
            content = f"{include_line}\n"

        sound_sources_pattern = re.compile(r'(\bORTSDefaultSoundSources\s*\((.*?)\))', re.IGNORECASE | re.DOTALL)
        match = sound_sources_pattern.search(content)
        
        new_lines_to_add = ""
        for sms_file in all_sms_files:
            if f'Include ( "..\\\\SOUND\\\\{sms_file}" )' not in content:
                new_lines_to_add += f'\tInclude ( "..\\\\SOUND\\\\{sms_file}" )\n'

        if match:
            if new_lines_to_add:
                self.log(f"[Debug] Appending {len(new_lines_to_add.strip().splitlines())} new sound(s) to existing ORTSDefaultSoundSources block.")
                new_block = match.group(1).rstrip().rstrip(')') + "\n" + new_lines_to_add + ")"
                content = content.replace(match.group(1), new_block)
        elif new_lines_to_add:
            self.log("[Debug] No ORTSDefaultSoundSources block found. Creating a new one.")
            new_block = f"\nORTSDefaultSoundSources (\n{new_lines_to_add})"
            content += new_block

        try:
            with open(override_trk_path, 'w', encoding=encoding) as f:
                f.write(content)
            self.log(f"[Success] TRK override file '{override_trk_path.name}' written successfully.")
            return True
        except Exception as e:
            self.log(f"[ERROR] Failed to write to TRK override file. Reason: {e}")
            return False

    def get_activities_for_route(self, route_path_str):
        self.log(f"[Debug] Searching for activities in: {route_path_str}")
        activities = {}; activities_path = Path(route_path_str) / "ACTIVITIES"
        if not activities_path.is_dir():
            self.log("[Info] No ACTIVITIES directory found for this route.")
            return {}
        ...
        self.log(f"[Info] Found {len(activities)} activities.")
        return activities
    def get_activity_details(self, act_path_str):
        self.log(f"[Debug] Getting details for activity: {act_path_str}")
        ...
        if path_id_match:
            details["path_id"] = path_id_match.group(1) or path_id_match.group(2)
            self.log(f"[Debug] Found PathID: {details['path_id']}")
        else: self.log("[Warning] PathID not found in activity file.")
        season_match = re.search(r'Season\s*\(\s*(\d)\s*\)', content, re.IGNORECASE)
        if season_match:
            details["season"] = int(season_match.group(1))
            self.log(f"[Debug] Found Season: {details['season']}")
        ...
        if weather_events: 
            ...
            self.log(f"[Debug] Found {len(weather_events)} existing weather events.")
        return details
    def find_route_start_location(self, route_data):
        self.log(f"[Debug] Finding start location for route: {route_data['id']}")
        ...
        if lat_match and lon_match:
            self.log("[Debug] Found ORTSLatitude/Longitude coordinates in .trk file."); return float(lat_match.group(1)), float(lon_match.group(1))
        rs_match = re.search(r'RouteStart\s*\(\s*(-?\d+)\s+(-?\d+)\s+(-?\d+\.?\d*)\s+(-?\d+\.?\d*)\s*\)', content, re.IGNORECASE)
        if rs_match:
            self.log("[Debug] Using RouteStart with GoodeProjection to calculate coordinates.")
        ...
        self.log(f"[ERROR] No coordinate source found for route."); return None, None
    def get_activity_path_coords(self, route_path_str, path_id):
        if not path_id: self.log("[Info] No PathID provided; cannot draw map path."); return []
        ...
        self.log(f"[Debug] Parsing activity path: {pat_path.name}")
        ...
        if not pdp_list or not path_indices:
            self.log(f"[Warning] No valid path data found in {pat_path.name}"); return []
    ...
    def modify_and_save_activity(self, original_path_str, act_events_content, date_obj=None, chaotic=False):
        self.log(f"[Debug] Modifying activity file: {original_path_str}")
        ...
        try:
            ...
            self.log(f"[Info] Created safe copy: {new_path.name}")
            ...
            if name_match:
                ...
                self.log(f"[Info] Renamed activity to: \"{new_activity_name}\"")
            ...
            self.log("[Debug] Writing new content to file...")
            ...
            self.log(f"[ERROR] CRITICAL: Error modifying or saving file. Reason: {e}")
            return None, f"Error saving files: {e}"

    def cleanup_generated_files(self, search_path_str):
        search_path = Path(search_path_str)
        self.log(f"[Debug] Starting cleanup scan in: '{search_path.resolve()}'")
        if not search_path.is_dir():
            self.log("[ERROR] Cleanup path is not a valid directory.")
            return 0, 0
        
        patterns = [
            f"**/*.{APP_SUFFIX}.*.act",
            "**/SOUND/WEATHERLINK_*.wav",
            "**/SOUND/WEATHERLINK_*.sms",
            "**/OpenRails/ssource.dat",
            "**/OpenRails/*.trk.bak",
            "**/OpenRails/*.trk"
        ]
        ...
        for pattern in patterns:
            ...
            self.log(f"[Debug] Searching for files with pattern: '{pattern}'")
            files_to_delete = list(search_path.glob(pattern))
            for f in files_to_delete:
                try:
                    if file_type == 'trk' or f.name.lower() == 'ssource.dat':
                        with open(f, 'r', encoding='utf-8-sig', errors='ignore') as check_f:
                            content = check_f.read().lower()
                            if 'weatherlink' not in content:
                                self.log(f"[Debug] Skipping non-WEATHERLINK config file: {f.name}")
                                continue
                    f.unlink()
                    deleted_counts.setdefault(file_type, 0)
                    deleted_counts[file_type] += 1
                    self.log(f"[Info] Deleted {file_type} file: {f.name}")
                except OSError as e:
                    # --- FIX: Added closing characters to the f-string ---
                    self.log(f"[ERROR] Could not delete {f.name}: {e}")
        
        total_deleted = sum(deleted_counts.values())
        self.log(f"[Debug] Cleanup complete for '{search_path.name}'. Deleted {total_deleted} file(s).")
        return deleted_counts.get('act', 0), total_deleted - deleted_counts.get('act', 0)