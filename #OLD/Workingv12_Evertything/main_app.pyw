...
class MainApplication(tk.Tk):
    ...
    def parse_and_display_existing_weather(self, details):
        ...
        for event_block in details['existing_weather']:
            ...
            def get_param(name):
                match = re.search(fr'{name}\s*\(\s*([\d\.]+)\s*(\d+)\s*\)', event_block)
                return match.groups() if match else (None, None)
            overcast, _ = get_param('ORTSOvercast'); fog, _ = get_param('ORTSFog'); precip, _ = get_param('ORTSPrecipitationIntensity'); liquid, _ = get_param('ORTSPrecipitationLiquidity')
            if time_val != 'N/A': self.existing_weather_human.insert('', 'end', values=(f"Event at {time_val}s", ""))
            if overcast: self.existing_weather_human.insert('', 'end', values=("  Overcast", f"{float(overcast)*100:.0f}% ({overcast})"))
            if fog: self.existing_weather_human.insert('', 'end', values=("  View Distance", f"{float(fog)/1000:.1f} km ({fog})"))
            if precip: self.existing_weather_human.insert('', 'end', values=("  Precipitation", f"{float(precip)*1000:.1f} mm/h ({precip})"))
            if liquid: self.existing_weather_human.insert('', 'end', values=("  Precip. Type", f"{'Rain' if float(liquid) > 0.5 else 'Snow'} ({liquid})"))
