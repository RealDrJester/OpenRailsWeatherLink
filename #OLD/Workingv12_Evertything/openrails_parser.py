...
class OpenRailsParser:
    ...
    def get_activity_details(self, act_path_str):
        ...
        path_id_match = re.search(r'PathID\s*\(\s*(?:"([^"]*)"|([0-9]*\.?[0-9]+))\s*(?:\s+[0-9]+)?\s*\)', content, re.IGNORECASE)
        ...
        # --- FIX: Capture EventCategoryTime OR EventTypeTime blocks with ORTSWeatherChange ---
    ...
    def get_activity_path_coords(self, route_path_str, path_id):
        ...
        path_coords = []
        sample_rate = max(1, len(path_indices) // 100)
