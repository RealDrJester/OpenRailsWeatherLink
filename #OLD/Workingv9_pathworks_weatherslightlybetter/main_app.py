...
class MainApplication(tk.Tk):
    ...
    def setup_ui(self):
        ...
        ew_human_frame = ttk.LabelFrame(ew_pane, text="Human-Readable", padding=5); self.existing_weather_human = ttk.Treeview(ew_human_frame, columns=('param', 'value'), show='headings'); self.existing_weather_human.heading('param', text='Parameter'); self.existing_weather_human.heading('value', text='Value'); self.existing_weather_human.column('param', width=120); self.existing_weather_human.pack(fill=tk.BOTH, expand=True); ew_pane.add(ew_human_frame, weight=1)
    ...
    def parse_and_display_existing_weather(self, details):
        ...
        self.existing_weather_human.insert('', 'end', values=("Season", f"{seasons.get(details['season'], 'Unknown')} ({details['season']})"))
        
        for event_block in details['existing_weather']:
            # Extract Time
            ...
            event_name = name_match.group(1).strip() if name_match else 'Unknown Event'
            ...
            if time_val != 'N/A':
                ...
                seconds = time_val % 60
                time_display = f"{hours:02d}:{minutes:02d}:{seconds:02d} ({time_val}s)"
                self.existing_weather_human.insert('', 'end', values=(f"Event: {event_name}", time_display))
            
            # Display weather parameters with human-readable values
            if overcast:
                overcast_percent = float(overcast) * 100
                duration_text = f" (over {overcast_duration}s)" if overcast_duration else ""
                self.existing_weather_human.insert('', 'end', values=("  Cloud Cover", f"{overcast_percent:.0f}%{duration_text}"))
            
            if fog:
                fog_km = float(fog) / 1000.0
                duration_text = f" (over {fog_duration}s)" if fog_duration else ""
                self.existing_weather_human.insert('', 'end', values=("  Visibility", f"{fog_km:.1f} km{duration_text}"))
            
            if precip:
                precip_mm = float(precip) * 1000.0
                duration_text = f" (over {precip_duration}s)" if precip_duration else ""
                self.existing_weather_human.insert('', 'end', values=("  Precipitation", f"{precip_mm:.1f} mm/h{duration_text}"))
            
            if liquid:
                liquid_val = float(liquid)
                precip_type = "Rain" if liquid_val > 0.5 else "Snow"
                duration_text = f" (over {liquid_duration}s)" if liquid_duration else ""
                self.existing_weather_human.insert('', 'end', values=("  Precip. Type", f"{precip_type} ({liquid_val}){duration_text}"))
    ...
    def _update_weather_widgets(self, data, date_obj):
        ...
        for i in range(min(24, len(hourly.get('time', [])))):
            temp = f"{hourly['temperature_2m'][i]:.1f}Ã‚Â°C" if hourly['temperature_2m'][i] is not None else "N/A"
