        ...
        try:
            for i in range(48):
                ...
                for sound_def in sound_manager.sound_definitions:
                    ...
                    condition = sound_def['condition']
                    
                    sound_channels.setdefault(category, 0)
                    
                    if condition in conditions:
                        if event_time_seconds >= sound_channels[category]:
                            if not sound_playlists.get(category) or not sound_playlists[category]:
                                all_sounds = sound_manager.sounds.get(category, [])
                                if all_sounds:
                                    shuffled_sounds = random.sample(all_sounds, len(all_sounds))
                                    sound_playlists[category] = shuffled_sounds
                            ...
                            if sound_playlists.get(category):
                                ...
                                if sound_filename_in_act:
                                    unique_id = f"9{global_sound_counter}"
                                    sound_events.extend([
                                        f"\t\tEventCategoryTime (", f"\t\t\tEventTypeTime ( )", f"\t\t\tID ( {unique_id} )", f"\t\t\tActivation_Level ( 1 )",
                                        f"\t\t\tName ( WTHLINK_{category}_{i}_{global_sound_counter} )", f"\t\t\tTime ( {schedule_time} )", f"\t\t\tOutcomes (",
                        ...
                        if event_time_seconds >= sound_channels[category]:
                            ...
                            if sound_playlists.get(category):
                                ...
                                if sound_filename_in_act:
                                    ...
                                    events.extend([
                                        f"\t\tEventCategoryTime (", f"\t\t\tEventTypeTime ( )", f"\t\t\tID ( {unique_id} )", f"\t\t\tActivation_Level ( 1 )",
                                        f"\t\t\tName ( WTHLINK_Chaotic_{category}_{i}_{global_sound_counter} )", f"\t\t\tTime ( {schedule_time} )", f"\t\t\tOutcomes (",
                                        f"\t\t\t\tORTSActivitySound ( ORTSActSoundFile ( \"{sound_filename_in_act}\" \"{sound_info['sound_type']}\" ) )",
                                        f"\t\t\t)", f"\t\t)"
                                    ])
                                    global_sound_counter += 1
