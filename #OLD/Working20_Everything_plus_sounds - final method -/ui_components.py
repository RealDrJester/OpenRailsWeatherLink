...
class SettingsWindow(tk.Toplevel):
    def __init__(self, parent, config_manager):
        ...
        ttk.Button(sound_frame, text="Rescan `user_sounds` Folder", command=self.parent.refresh_sound_files).pack(fill="x")
...
class ManualWeatherEditor(tk.Toplevel):
    def __init__(self, parent):
        ...
        ttk.Label(self, text="Manual Weather Editor is a planned feature.\n\nThis window is a placeholder for future development.", justify="center", padding=20).pack(expand=True)
...
class ForecastChart(ttk.Frame):
    ...
    def plot_data(self, hourly_data, date_str):
        ...
        vis = [v / 1000 for v in hourly_data['visibility'][:24]] if hourly_data.get('visibility') else [0]*24
