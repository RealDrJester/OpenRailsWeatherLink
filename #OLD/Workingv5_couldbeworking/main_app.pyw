...
class MainApplication(tk.Tk):
    ...
    def setup_ui(self):
        ...
        existing_weather_frame = ttk.Frame(details_notebook); self.existing_weather_text = scrolledtext.ScrolledText(existing_weather_frame, wrap=tk.WORD, state=tk.DISABLED, font=("Courier New", 9)); self.existing_weather_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5); details_notebook.add(existing_weather_frame, text="Existing Weather")
        ...
        log_frame_tab = ttk.Frame(details_notebook); self.log_text = scrolledtext.ScrolledText(log_frame_tab, wrap=tk.WORD, state=tk.DISABLED, font=("Courier New", 8), height=8); self.log_text.pack(fill=tk.BOTH, expand=True); details_notebook.add(log_frame_tab, text="Debug Console")
    ...
    def load_activities_for_route(self, route_info):
        self.after(0, self.start_loading, f"Loading activities for {route_info['id']}..."); self.current_activities = self.parser.get_activities_for_route(route_info['path']); self.after(0, self._populate_activities_list); self.after(0, self.stop_loading)
    ...
    def update_activity_info(self):
        details = self.parser.get_activity_details(self.selected_activity_path); self.after(0, lambda: self.update_details_text(details))
        route_info = self.current_route_data.get(self.route_listbox.get(self.route_listbox.curselection()[0]))
        path_coords = self.parser.get_activity_path_coords(route_info['path'], details['path_id'])
        if path_coords:
    ...
    def _update_map_with_path(self, path_coords):
        ...
        if len(path_coords) >= 1:
            ...
            if len(path_coords) > 1:
                ...
                self.after(250, lambda: self.map_widget.fit_bounding_box((max_lat, min_lon), (min_lat, max_lon)))
            else: self.map_widget.set_zoom(10)
    ...
    def update_details_text(self, details):
        ...
        self.existing_weather_text.config(state=tk.NORMAL); self.existing_weather_text.delete(1.0, tk.END); self.existing_weather_text.insert(tk.END, details['existing_weather']); self.existing_weather_text.config(state=tk.DISABLED)
    ...
    def generate_weather_worker(self):
        ...
        weather_events, msg = self.weather.create_weather_events_string(lat, lon, date_obj, start_hour)
        ...
        self.log(f"  > {msg}")
        route_info = self.current_route_data.get(self.route_listbox.get(self.route_listbox.curselection()[0]))
        new_path, msg = self.parser.modify_and_save_activity(self.selected_activity_path, weather_events, date_obj)
    ...
    def clear_info(self):
        ...
        self.existing_weather_text.config(state=tk.NORMAL); self.existing_weather_text.delete(1.0, tk.END); self.existing_weather_text.config(state=tk.DISABLED)
