...
class MainApplication(tk.Tk):
    ...
    def load_activities_for_route(self, route_info):
        ...
        self.parser.load_track_nodes_for_route(route_info['trk_path'])
        self.current_activities = self.parser.get_activities_for_route(route_info['path'])
        self.after(0, self._populate_activities_list); self.after(0, self.stop_loading)
    ...
    def update_activity_info(self):
        ...
        if path_coords:
            self.found_coords = path_coords[0]
            self.after(0, lambda: self._update_map_with_path(path_coords))
            self.update_weather_info(self.found_coords[0], self.found_coords[1])
        else:
            lat, lon = self.parser.find_route_start_location(route_info)
            self.found_coords = (lat, lon)
            if lat is not None:
                self.after(0, lambda: self._update_map_with_start_point(lat, lon))
    ...
    def _update_map_with_start_point(self, lat, lon):
        self.map_widget.delete_all_path(); self.map_widget.delete_all_marker()
        self.map_widget.set_marker(lat, lon, text="Activity Start (No Path Data)")
        self.map_widget.set_position(lat, lon, zoom=10)
    ...
    def _update_weather_widgets(self, lat, lon, data):
        ...
        self.forecast_text.config(state=tk.NORMAL); self.forecast_text.delete(1.0, tk.END); self.forecast_text.insert(tk.END, header + "-"*(len(header)+5) + "\n")
        for i in range(min(12, len(hourly.get('time', [])))):
            temp = f"{hourly['temperature_2m'][i]:.1f}°C"; precip = f"{hourly['precipitation'][i]:.1f}mm"; clouds = f"{hourly['cloudcover'][i]}%"; wind = f"{hourly['windspeed_10m'][i]:.1f}km/h"; cond = self.weather.WMO_CODES.get(hourly['weathercode'][i], 'N/A')
            line = f"{hourly['time'][i][-5:]:<10}{temp:<10}{precip:<12}{clouds:<10}{wind:<15}{cond}\n"
    ...
    def generate_weather_worker(self):
        ...
        self.after(0, self.stop_loading); self.after(0, lambda: self.generate_button.config(state=tk.NORMAL))
