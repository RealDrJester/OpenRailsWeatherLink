...
class OpenRailsParser:
    ...
    def load_track_nodes_for_route(self, trk_path_str):
        self.current_track_nodes = {}
        # This function is now a fallback, path data is in .pat files for Level 2
        pass
    ...
    def get_activities_for_route(self, route_path_str):
        ...
        for act_path in sorted(activities_path.glob("*.act")):
            ...
            if name_match: activities[act_path.name] = {"display_name": name_match.group(1), "path": str(act_path)}
        return activities
        
    def get_activity_details(self, act_path_str):
        details = {"description": "N/A", "briefing": "N/A", "path_id": None}
    ...
    def get_activity_path_coords(self, route_path_str, path_id):
        ...
        # New parsing logic for TrackPDP and TrPathNode
        ...
        pdp_list = [
            {'tile_x': int(m[1]), 'tile_z': int(m[2]), 'offset_x': float(m[3]), 'offset_z': float(m[5])}
            for m in pdp_pattern.finditer(content)
        ]
        ...
        path_coords = []
        sample_rate = max(1, len(path_indices) // 100) # Max 100 points for performance
