...
class MainApplication(tk.Tk):
    ...
    def show_manual_editor(self):
        if not self.selected_activity_path:
            ...
            return
        dialog = ManualWeatherEditor(self)
        self.wait_window(dialog)
        if dialog.result_string:
            self.run_generation_thread(manual_events=dialog.result_string)
    ...
    def generate_weather_worker(self, chaotic=False, preset_name=None, manual_events=None):
        ...
        if lat is None and not manual_events: self.log("[ERROR] Generation failed: Coordinates not found."); self.after(0, self.stop_loading); self.after(0, lambda: self.generate_button.config(state=tk.NORMAL)); return
