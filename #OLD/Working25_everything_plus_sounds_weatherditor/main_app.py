...
class MainApplication(tk.Tk):
    ...
    def generate_weather_worker(self, chaotic=False, preset_name=None):
        ...
        if lat is None and not (chaotic or (preset_name and preset_name != "Select a preset...")):
        ...
        # --- FIX: Ensure 'season' is defined for all logical paths before it's needed ---
        date_obj = None
        ...
        if not weather_events:
            self.log(f"[ERROR] Weather event string creation failed: {msg}")
            self.after(0, self.stop_loading)
            return
        self.log(f"[Info] {msg}")
        
        new_path, msg = self.parser.modify_and_save_activity(self.selected_activity_path, weather_events, date_obj, chaotic, manual_suffix=None, season=season)
        if new_path:
        ...
        self.after(0, self.stop_loading)
        self.after(0, self.on_route_select)
