...
class MainApplication(tk.Tk):
    ...
    def run_global_cleanup(self):
        if not self.parser: 
            messagebox.showwarning("Warning", "Please select a Content Folder first.")
            return

        msg = (f"This will scan ALL content folders and permanently delete ALL generated files:\n"
               f"  - *.WTHLINK.*.act\n"
               f"  - WEATHERLINK_*.wav\n\n"
        ...
        if messagebox.askyesno("Confirm GLOBAL Cleanup", msg, icon='warning'):
            ...
            for path in self.config.get('content_paths'):
                # We need a parser instance for the path to clean up
