...
class OpenRailsParser:
    ...
    def get_activity_path_coords(self, route_path_str, path_id):
        ...
        # --- FIX: Restore original, working regex for TrPathNode that does NOT capture distance ---
        ...
        path_coords = []; sample_rate = max(1, len(path_indices) // 100); total_dist = 0 # Distance info not available in this format.
        for i, pdp_index in enumerate(path_indices):
            if i % sample_rate == 0 or i == len(path_indices) - 1:
                if pdp_index < len(pdp_list):
                    ...
                    if lat is not None: path_coords.append(((lat, lon), total_dist)) # Distance is dummy here, needs real calculation if available
        self.log(f"[INFO] Generated map path with {len(path_coords)} points.")
        return path_coords, total_dist # Total distance is not reliably calculated from this file type.
