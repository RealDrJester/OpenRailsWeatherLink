...
class WeatherService:
    ...
    def get_weather_data(self, weather_points, date_obj=None, average_results=True):
        ...
        if date_obj is not None:
            ...
            # For both archive and forecast, getting the next day ensures a full 48 hours of data for interpolation
        ...
        avg_data = {"hourly": {}}
        if 'hourly' not in all_results[0] or 'time' not in all_results[0]['hourly']:
            self.log(f"[ERROR] Invalid API response structure received. First result: {all_results[0]}")
            return None

        hourly_keys = all_results[0]['hourly'].keys()
        num_hours = len(all_results[0]['hourly']['time'])
        ...
        for key in hourly_keys:
            ...
            for i in range(num_hours):
                ...
                for d in all_results:
                    if d and d.get('hourly') and d['hourly'].get(key) and i < len(d['hourly'][key]) and d['hourly'][key][i] is not None:
                        values.append(d['hourly'][key][i])
                
                if values:
                    avg_value = sum(values) / len(values)
                    avg_data['hourly'][key].append(avg_value if key != 'weathercode' else int(round(avg_value)))
                else:
                    avg_data['hourly'][key].append(None)
        
        self.log(f"[Info] Successfully fetched and averaged weather data from {len(all_results)} of {len(weather_points)} locations.")
        return avg_data
    ...
    def create_weather_events_string(self, path_coords, path_dist, season, force_snow, date_obj, start_hour, add_thunder_sounds, add_wind_sounds, add_rain_sounds, sound_manager, route_path):
        ...
        weather_data_list = self.get_weather_data(weather_points, date_obj, average_results=False)
